        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ color: '#6B7280', fontSize: '0.875rem', margin: '3px 0 0', fontWeight: '500' }}>
              {profile?.email || user?.email} · Your portfolio at a glance
            </p>
          </div>
                ...
                {PORTFOLIO_ASSETS.map((a) => (
                  <div key={a.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 0', borderBottom: '1px solid #F9FAFB' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: a.bg, color: a.color, fontWeight: '800', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{a.initial}</div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      {a.change !== null
                        ...
                        : <span style={{ color: '#111827', fontWeight: '700', fontSize: '0.9375rem' }}>${a.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                      }
                    </div>
                  </div>
                ))}
                  ...
                  <thead>
                    <tr>
                      {['Asset', 'Price', '24h', 'Market Cap'].map(h => (
                        <th key={h} style={{ textAlign: h === 'Asset' ? 'left' : 'right', padding: '8px 10px', fontSize: '0.7rem', fontWeight: '700', color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.07em', borderBottom: '1px solid #F3F4F6' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {cryptoData.map((coin, i) => (
                      <tr key={coin.id} onClick={() => navigate(`/asset/${coin.id}`)} style={{ cursor: 'pointer' }} onMouseEnter={e => { e.currentTarget.style.background = '#F9FAFB'; }} onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}>
                        <td style={{ padding: '12px 10px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{ width: 32, height: 32, borderRadius: '50%', background: `hsl(${i * 36},68%,92%)`, color: `hsl(${i * 36},55%,42%)`, fontWeight: '800', fontSize: '0.6875rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{coin.symbol.slice(0, 3)}</div>
                            <div>
                        ...
                        <td style={{ textAlign: 'right', padding: '12px 10px', fontWeight: '700', color: '#111827', fontSize: '0.875rem' }}>
                          <span key={coin._tick} className={coin._dir >= 0 ? 'price-flash-up' : 'price-flash-down'}>${coin.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </td>
                        ...
                        <td style={{ textAlign: 'right', padding: '12px 10px', color: '#6B7280', fontSize: '0.8125rem' }}>${(coin.marketCap / 1e9).toFixed(1)}B</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Reveal delay={60}>
            <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #E5E7EB', padding: '24px' }}>
              <h2 style={{ fontSize: '1.0625rem', fontWeight: '800', color: '#111827', margin: '0 0 16px' }}>My Profile</h2>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '16px' }}>
                <div style={{ width: 48, height: 48, borderRadius: '50%', background: '#EFF4FF', color: '#1652F0', fontWeight: '800', fontSize: '1.125rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {(profile?.name || user?.name || 'U')[0].toUpperCase()}
                </div>
                <div>
                  <p style={{ margin: 0, fontWeight: '700', color: '#111827', fontSize: '0.9375rem' }}>{profile?.name || user?.name || '—'}</p>
                  <p style={{ margin: 0, color: '#6B7280', fontSize: '0.8125rem' }}>{profile?.email || user?.email}</p>
                </div>
              </div>
              <div style={{ background: '#F9FAFB', borderRadius: '10px', padding: '12px 14px', fontSize: '0.8125rem', color: '#6B7280' }}>
          ...
          <Reveal delay={120}>
            <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #E5E7EB', padding: '24px' }}>
              ...
              <div style={{ display: 'flex', background: '#F3F4F6', borderRadius: '12px', padding: '4px', marginBottom: '18px' }}>
                {['buy', 'sell'].map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)} style={{ flex: 1, padding: '8px', borderRadius: '9px', border: 'none', cursor: 'pointer', fontWeight: '700', fontSize: '0.875rem', background: activeTab === tab ? '#fff' : 'transparent', color: activeTab === tab ? '#111827' : '#6B7280', transition: 'all 0.15s', textTransform: 'capitalize' }}>
                    {tab.charAt(0).toUpperCase() + tab.slice(1)}
                  </button>
                ))}
              </div>
              <label style={{ display: 'block', color: '#374151', fontWeight: '600', fontSize: '0.8125rem', marginBottom: '6px' }}>Asset</label>
              <select value={selectedCoin} onChange={e => setSelectedCoin(e.target.value)} style={{ width: '100%', padding: '11px 14px', borderRadius: '10px', border: '1.5px solid #E5E7EB', fontSize: '0.9375rem', fontWeight: '600', color: '#111827', marginBottom: '14px', background: '#fff', cursor: 'pointer', outline: 'none', boxSizing: 'border-box' }}>
                {cryptoData.map(c => <option key={c.id} value={c.id}>{c.name} ({c.symbol})</option>)}
              </select>
              <label style={{ display: 'block', color: '#374151', fontWeight: '600', fontSize: '0.8125rem', marginBottom: '6px' }}>Amount (USD)</label>
              <div style={{ position: 'relative', marginBottom: '14px' }}>
                <span style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: '#9CA3AF', fontWeight: '700', pointerEvents: 'none' }}>$</span>
                <input type="number" min="0" placeholder="0.00" value={buyAmount} onChange={e => setBuyAmount(e.target.value)} style={{ width: '100%', padding: '11px 14px 11px 28px', borderRadius: '10px', border: '1.5px solid #E5E7EB', fontSize: '0.9375rem', color: '#111827', outline: 'none', boxSizing: 'border-box', fontWeight: '600', background: '#fff' }} />
              </div>
              {estimatedReceive && (
              ...
              <button style={{ width: '100%', padding: '13px', background: activeTab === 'buy' ? '#1652F0' : '#EF4444', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '800', fontSize: '1rem', cursor: 'pointer', transition: 'opacity 0.15s' }} onMouseEnter={e => { e.currentTarget.style.opacity = '0.88'; }} onMouseLeave={e => { e.currentTarget.style.opacity = '1'; }}>
                {activeTab === 'buy' ? `Buy ${selectedCoinData?.symbol ?? ''}` : `Sell ${selectedCoinData?.symbol ?? ''}`}
              </button>
              {selectedCoinData && <p style={{ textAlign: 'center', fontSize: '0.8125rem', color: '#9CA3AF', margin: '10px 0 0', fontWeight: '600' }}>1 {selectedCoinData.symbol} = ${selectedCoinData.price.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>}
            </div>
          </Reveal>
          ...
          <Reveal delay={160}>
            <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #E5E7EB', padding: '24px' }}>
              ...
              {RECENT_TXS.map((tx, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: i < RECENT_TXS.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <p style={{ margin: 0, fontWeight: '700', color: '#111827', fontSize: '0.875rem' }}>${tx.value.toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
