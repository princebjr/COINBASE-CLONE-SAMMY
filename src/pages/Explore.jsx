        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px 0', display: 'grid', gridTemplateColumns: '1fr 296px', gap: '28px', alignItems: 'start' }} className="explore-hero-grid">
          ...
          <div>
            ...
            <div style={{ marginBottom: '18px' }} className="hero-anim-up">
              <h1 style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: '500', color: '#111827', letterSpacing: '-0.025em', margin: '0 0 5px' }}>
                Explore crypto
              </h1>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.875rem', color: '#6B7280', flexWrap: 'wrap' }}>
                ...
                <span style={{ color: '#DC2626', fontWeight: '500', display: 'inline-flex', alignItems: 'center', gap: '2px' }}>
                  ↘ 1.18%
                </span>
                <span>(24hrs)</span>
            ...
            <div style={{ paddingBottom: '28px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <h2 style={{ fontSize: '1.125rem', fontWeight: '500', color: '#111827', margin: 0, letterSpacing: '-0.02em' }}>Market stats</h2>
                <NavArrows
                ...
                {visibleStats.map((stat, i) => (
                  <div key={stat.label} style={{ background: '#F9FAFB', borderRadius: '12px', overflow: 'hidden', border: '1px solid #F3F4F6' }}>
                    <div style={{ padding: '12px 12px 4px' }}>
                      ...
                      <p style={{ fontSize: '0.9375rem', fontWeight: '500', color: '#111827', margin: '0 0 2px', letterSpacing: '-0.02em' }}>{stat.value}</p>
                      <p style={{ fontSize: '0.8125rem', fontWeight: '500', color: stat.change >= 0 ? '#16A34A' : '#DC2626', margin: 0 }}>
          ...
          <div ref={sidebarRef} style={{ display: 'flex', flexDirection: 'column', gap: '16px', paddingBottom: '28px' }} className="reveal reveal-right">
            ...
            <div style={{ background: '#1652F0', borderRadius: '16px', padding: '22px', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: '1.0625rem', fontWeight: '500', color: '#fff', margin: '0 0 5px', paddingRight: '86px' }}>Get started</p>
              <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.82)', margin: '0 0 18px', lineHeight: '1.5' }}>Create your account today</p>
              <Link to="/signup" style={{ display: 'inline-block', background: '#fff', color: '#111827', fontWeight: '500', fontSize: '0.875rem', padding: '9px 22px', borderRadius: '9px', textDecoration: 'none' }}
            ...
            <div style={{ border: '1px solid #E5E7EB', borderRadius: '16px', padding: '18px', background: '#fff' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3px' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: '500', color: '#111827', margin: 0 }}>Top movers</h3>
                <NavArrows
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: col, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ color: '#fff', fontWeight: '500', fontSize: '11px' }}>{crypto.symbol.slice(0,2).toUpperCase()}</span>
                      </div>
                      <p style={{ fontSize: '0.8125rem', fontWeight: '500', color: '#111827', margin: 0 }}>{crypto.symbol}</p>
                      <p style={{ fontSize: '0.9375rem', fontWeight: '500', color: isUp ? '#16A34A' : '#DC2626', margin: 0 }}>
        ...
        <div ref={tableRef} style={{ border: '1.5px solid #E5E7EB', borderRadius: '16px', overflow: 'hidden' }} className="reveal reveal-fade-up">
                      ...
                      <td style={{ padding: '16px 20px', whiteSpace: 'nowrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ width: 40, height: 40, borderRadius: '50%', background: iconColor, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <span style={{ color: '#fff', fontWeight: '500', fontSize: '13px' }}>{crypto.symbol.slice(0,2).toUpperCase()}</span>
                          </div>
                          <div>
                            <div style={{ fontSize: '0.9375rem', fontWeight: '500', color: '#111827' }}>{crypto.name}</div>
                            <div style={{ fontSize: '0.8125rem', color: '#6B7280' }}>{crypto.symbol}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: '16px 20px', whiteSpace: 'nowrap', fontSize: '0.9375rem', fontWeight: '500', color: '#111827', fontVariantNumeric: 'tabular-nums' }}>
                        <span key={crypto._tick} className={crypto._dir >= 0 ? 'price-flash-up' : 'price-flash-down'}>
                          {fmt(crypto.price)}
                        </span>
                      </td>
                      <td style={{ padding: '16px 20px', whiteSpace: 'nowrap' }}>
                        <span style={{ fontSize: '0.875rem', fontWeight: '500', color: isUp ? '#16A34A' : '#DC2626', background: isUp ? '#F0FDF4' : '#FEF2F2', padding: '3px 10px', borderRadius: '99px' }}>
                          {isUp ? '+' : ''}{crypto.change24h.toFixed(2)}%
                        </span>
                      </td>
                      <td style={{ padding: '16px 20px', whiteSpace: 'nowrap', fontSize: '0.9375rem', fontWeight: '500', color: '#374151' }} className="hide-mobile-col">
                        {fmtMC(crypto.marketCap)}
                      </td>
                      <td style={{ padding: '16px 20px', whiteSpace: 'nowrap', textAlign: 'right' }}>
                        <Link to={`/asset/${crypto.id}`} style={{ fontSize: '0.8125rem', fontWeight: '500', color: '#1652F0', textDecoration: 'none', background: '#EFF6FF', padding: '6px 14px', borderRadius: '8px' }}>
                          Trade
                        </Link>
                      </td>
                    </tr>
          ...
          {filteredCrypto.length === 0 && (
            <div style={{ textAlign: 'center', padding: '56px 24px' }}>
              ...
              <p style={{ fontSize: '1.0625rem', fontWeight: '500', color: '#374151', margin: '0 0 6px' }}>No results found</p>
